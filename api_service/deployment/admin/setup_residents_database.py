...
class DatabaseSetup:
    ...
    def get_connection_config(self) -> dict:
        ...
        # Add appropriate admin/master connections based on type
        if server_info["type"] == "local":
            config["master_connection"] = f"mssql+pyodbc://{server_info['server']}/master?driver=ODBC+Driver+17+for+SQL+Server&TrustServerCertificate=yes&Trusted_Connection=yes"
            config["db_connection"] = get_admin_connection_string()
        else:  # azure
            # For Azure, we need admin credentials - prompt user
            admin_username = input("Enter Azure SQL admin username: ").strip()
            admin_password = getpass.getpass("Enter Azure SQL admin password: ")
            
            if not all([admin_username, admin_password]):
                print("Azure SQL admin credentials are required.")
                sys.exit(1)
            
            from urllib.parse import quote_plus
            encoded_admin_password = quote_plus(admin_password)
            
            config["admin_username"] = admin_username
            config["admin_password"] = admin_password
            config["master_connection"] = f"mssql+pyodbc://{admin_username}:{encoded_admin_password}@{server_info['server']}/master?driver=ODBC+Driver+17+for+SQL+Server&TrustServerCertificate=yes"
            config["db_connection"] = f"mssql+pyodbc://{admin_username}:{encoded_admin_password}@{server_info['server']}/{self.database_name}?driver=ODBC+Driver+17+for+SQL+Server&TrustServerCertificate=yes"
