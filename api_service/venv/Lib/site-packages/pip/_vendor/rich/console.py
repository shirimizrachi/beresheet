...
class Console:
    ...
    def export_svg(
        ...
        *,
        ...
        Args:
            ...
            firebase_id (str, optional): unique id that is used as the prefix for various elements (CSS styles, node
                ...
                if text != " " * len(text):
                    text_group.append(
                        make_tag(
                            "text",
                            escape_text(text),
                            _class=f"{firebase_id}-{class_name}",
                            x=x * char_width,
                            y=y * line_height + char_height,
                            textLength=char_width * len(text),
                            clip_path=f"url(#{firebase_id}-line-{y})",
                        )
                    )
                x += cell_len(text)
        ...
        lines = "\n".join(
            f"""<clipPath id="{firebase_id}-line-{line_no}">
    {make_tag("rect", x=0, y=offset, width=char_width * width, height=line_height + 0.25)}
        ...
        styles = "\n".join(
            f".{firebase_id}-r{rule_no} {{ {css} }}" for css, rule_no in classes.items()
        )
